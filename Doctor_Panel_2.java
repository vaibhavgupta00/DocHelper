/ * This Module use to Operate all the operation is related to data patients.
...
spreadsheet = workbook.createSheet("Patients Details");
 ...
 //---------------------Image write--------------------//
   InputStream inputStream = new(rs.getString(j));
...
para= new Paragraph();
 ...
 img.scaleToFit(106, 50);
        ...
        para.add("Pid-->"+rs.getInt(1)+"\nPName-->"+rs.getString(3)+"\nPAge-->"+rs.getInt(4)+"\nPSex-->"+rs.getString(5)+"\nPAddress-->"+rs.getString(6)+"\nPDate-->"+rs.getString(7)+"\nPEmail-->"+rs.getString(9)+"\nPMobileNo.-->"+rs.getString(8));
        	...
        	if(newPage==2) {
       newPage=-1;
// reset the page write action .
        fno=708f;
    	doc.newPage();
// Use to write new page in pdf file. 
        	}
 ...
 * who is similar to input string ( this operation wrok for searching suggestion of name). */
					}
					}
if(nameList.isEmpty()) {
